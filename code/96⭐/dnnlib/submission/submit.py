...
class SubmitConfig(util.EasyDict):
    ...
    def __init__(self):
        ...
        # run (set these)
        self.run_dir_root = ""        #运行的根目录，{str} 'results'
        self.run_desc = ""           # 其生在'run_dir_root'目录下生成子项目的名称  {str} 'sgan-result-1gpu'
        self.run_dir_ignore = ["__pycache__", "*.pyproj", "*.sln", "*.suo", ".cache", ".idea", ".vs", ".vscode"] # 应该是忽略拷贝的文件
        self.run_dir_extra_files = None

        # submit (set these)
        self.submit_target = SubmitTarget.LOCAL
        self.num_gpus = 1                       # 使用GPU的数目
        self.print_info = False                # 是否打印信息
        self.ask_confirmation = False

        # (automatically populated)
        self.run_id = None               # 其生成子项目名称的前缀，如00002-sgan-result-1gpu前面的00002
        self.run_name = None            # 子项目的名称{str} '00011-sgan-result-1gpu'
        self.run_dir = None            # 子项目运行的目录{str} 'results\\00011-sgan-result-1gpu'
        self.run_func_name = None     # 训练运行的函数，在training.training_loop.training_loop
        self.run_func_kwargs = None  # 运行函数的参数，后续着重分析，就是training.training_loop.training_loop函数的参数
        self.user_name = None       # 电脑用户名
        self.task_name = None      # 该次程序运行吗，名称（胡乱理解就行，无关紧要）
        self.host_name = "localhost"    # 电脑主机名
