...
def parse_tfrecord_tf(record):
    features = tf.parse_single_example(record, features={
        'shape': tf.FixedLenFeature([4], tf.int64), # 注
...
class TFRecordDataset:
    def __init__(self,
        ...
        self.resolution_log2 = int(np.log2(self.resolution/6)+2)
        self.shape = [max_shape[0], self.resolution, self.resolution, self.resolution]
        for shape in tfr_shapes:
            print('dataset95',shape)
        tfr_lods = [self.resolution_log2 - int(np.log2(shape[1]/6)+1) for shape in tfr_shapes]
        print('dataset97', tfr_lods, self.resolution_log2, self.shape[1])
        assert all(shape[0] == max_shape[0] for shape in tfr_shapes)
        assert all(shape[1] == shape[2] for shape in tfr_shapes)
        assert all(shape[2] == shape[3] for shape in tfr_shapes)
        for shape, lod in zip(tfr_shapes, tfr_lods):
            print('dataset102',shape, lod)
        assert all(shape[1]/6*4 == self.resolution // (2**lod) for shape, lod in zip(tfr_shapes, tfr_lods))
        assert all(lod in tfr_lods for lod in range(self.resolution_log2 - 1))

        # Load labels.
        assert max_label_size == 'full' or max_label_size >= 0
        self._np_labels = np.zeros([1<<20, 0], dtype=np.float32)#1<<20会影响内存，不能1<<30。分配空间？
        ...
        with tf.name_scope('Dataset'), tf.device('/cpu:0'):
            ...
            for tfr_file, tfr_shape, tfr_lod in zip(tfr_files, tfr_shapes, tfr_lods):
                if tfr_lod < 0:
                    continue
                dset = tf.data.TFRecordDataset(tfr_file, compression_type='', buffer_size=buffer_mb<<20)#dataset = tf.data.TFRecordDataset(filenames) 创建dataset
...
class SyntheticDataset:
    def __init__(self, resolution=1024, num_channels=3, dtype='uint8', dynamic_range=[0,255], label_size=0, label_dtype='float32'):
        ...
        self.resolution_log2    = int(np.log2(resolution/6)+2)
...
def load_dataset(class_name='training.dataset.TFRecordDataset', data_dir=None, verbose=False, **kwargs):
    ...
    print('adjusted_kwargs=',adjusted_kwargs,'kwargs=',kwargs)
