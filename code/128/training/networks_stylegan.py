...
def G_mapping(
    latents_in,                             # 第一个输入：Z码向量 [minibatch, latent_size].
    labels_in,                              # 第二个输入：条件标签 [minibatch, label_size].
    latent_size             = 256,          # 潜在向量（Z）维度。
    label_size              = 0,            # 标签尺寸，0表示没有标签。
    dlatent_size            = 256,          # 解缠后的中间向量 (W) 维度。
...
def G_synthesis(
    ...
    close_deep_noise_x  = 12,           # 从哪层开始关闭？
    close_deep_style    = False,        # 关闭深层噪声？
    close_deep_style_x  = 12,           # 从哪层开始关闭？
    ...
    **_kwargs):                         # 忽略无法识别的关键字参数。
    ...
    num_layers = resolution_log2 * 3 - 3
    ...
    if use_noise:
        for layer_idx in range(num_layers):
            res = layer_idx // 3 + 2
            shape = [1, use_noise, 2 ** res, 2 ** res, 2 ** res]
    ...
    with tf.variable_scope('4x4x4'):
        ...
        with tf.variable_scope('Convs1'):
            x = layer_epilogue(conv2d(x, fmaps=nf(1), kernel=3, gain=gain, use_wscale=use_wscale), 2)

    def block(res, x):
        with tf.variable_scope('%dx%dx%d' % (2**res, 2**res, 2**res)):
            with tf.variable_scope('Conv0_up'):
                x = layer_epilogue(blur(upscale2d_conv2d(x, fmaps=nf(res-1), kernel=3, gain=gain, use_wscale=use_wscale, fused_scale=fused_scale)), res*3-6)
            with tf.variable_scope('Conv1'):
                x = layer_epilogue(conv2d(x, fmaps=nf(res-1), kernel=3, gain=gain, use_wscale=use_wscale), res*3-5)
            with tf.variable_scope('Conv2'):
                x = layer_epilogue(conv2d(x, fmaps=nf(res-1), kernel=3, gain=gain, use_wscale=use_wscale), res*3-4)
            return x
    ...
    def block(x, res):
        with tf.variable_scope('%dx%dx%d' % (2 ** res, 2 ** res, 2 ** res)):
            if res >= 3:  # 8x8x8分辨率及以上
                ...
                with tf.variable_scope('Convf0'):
                    x = act(apply_bias(conv2d(x, fmaps=nf(res - 1), kernel=3, gain=gain, use_wscale=use_wscale)))
                with tf.variable_scope('Convf1'):
                    x = act(apply_bias(conv2d(x, fmaps=nf(res - 1), kernel=3, gain=gain, use_wscale=use_wscale)))
